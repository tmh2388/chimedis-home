...
export async function verifyFirebaseToken(req, res, next) {
  ...
  try {
    ...
    const identities = decoded.firebase?.identities || {}; // { 'google.com': ['sub'], 'facebook.com': [...], email: [...] }
    // subject bền vững của provider hiện tại (KHÔNG phải email trừ khi password).
    let providerSubject = decoded.uid;
    if (provider === 'password') providerSubject = decoded.email || decoded.uid;
    else if (identities[provider]?.[0]) providerSubject = identities[provider][0];
    // MỌI provider đã liên kết vào tài khoản này (để /sync ghi đủ user_identities).
    const linked = [];
    for (const [p, subs] of Object.entries(identities)) {
      if (p === 'email') continue; // 'email' là danh sách email, không phải provider
      for (const sub of (Array.isArray(subs) ? subs : [subs])) {
        linked.push({ provider: p, provider_subject: String(sub) });
      }
    }
    if (provider === 'password' && !linked.some((l) => l.provider === 'password')) {
      linked.push({ provider: 'password', provider_subject: decoded.email || decoded.uid });
    }
    req.firebaseUser = {
      uid: decoded.uid,
      ...
      provider,               // 'password' | 'google.com' | 'apple.com' | 'facebook.com' | 'oidc.orcid' | ...
      provider_subject: String(providerSubject),
      linked,                 // [{ provider, provider_subject }]
    };
    next();
  } catch (err) {
