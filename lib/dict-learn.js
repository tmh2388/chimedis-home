 ...
 * @param {{mode?:'discovery'|'evidence'}} [opts]
 * @returns {Promise<typeof ex>}  ex đã bổ sung (nếu dịch thêm được)
 *
 * D6 (review PR#4): LLM CHỈ sinh candidate cho Discovery. Với Evidence Search, KHÔNG
 * lấp cụm chưa dịch bằng LLM — giữ `needs_resolution` để người dùng chọn nghĩa. Nhưng
 * VẪN ghi nhận miss (dict_term_misses làm việc đó ở tầng route), không gọi LLM ở đây.
 */
export async function enrichUntranslated(rawQuery, ex, opts = {}) {
  try {
    if (!isDbConfigured()) return ex;
    if (opts.mode === 'evidence') return ex; // Evidence: không lấp bằng LLM (reviewer PR#4)
    ...
    for (const r of resolved) {
      // D6: nhãn `medium` + viaLLM → UI hiện chip vàng "tự động — kiểm lại"; vào hàng đợi G4.
      ex.expandedFrom.push({ vi: r.term, en: r.en, syn: r.syn || [], viaLLM: true, confidence: 'medium' });
