...
// M3 slice 1b — cho phép "Lưu vào thư viện" từ BẤT KỲ mặt tìm kiếm nào (kể cả trang tìm
// công khai /index.html, vốn không chạy qua writeSearchRun()/connector pipeline của
// workbench nên record chưa từng được upsert). Dùng lại đúng logic dedup identity-graph
// (upsertRecord) nhưng tự mở/đóng connection riêng — KHÔNG nằm trong transaction search-run.
export async function upsertStandaloneRecord(rec) {
  if (!isDbConfigured()) throw new Error('Database chưa được cấu hình');
  const pool = getPool();
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    // allowUpdate: true — đường lưu thủ công/nhập RIS-BibTeX là hành động NGƯỜI DÙNG
    // CHỦ ĐỘNG re-save/re-import, khác với writeSearchRun() (bất biến theo M0). Nếu
    // không cho phép cập nhật, 1 lần import lỗi (parser cũ cắt cụt abstract...) sẽ "đóng
    // băng" vĩnh viễn bản ghi đó — xoá khỏi thư viện dự án rồi nhập lại cùng DOI vẫn ra
    // y hệt dữ liệu cũ vì identity-graph chỉ tìm thấy record_id có sẵn rồi dừng, không
    // bao giờ ghi đè (bug thật đã gặp, 2026-09-12).
    const recordId = await upsertRecord(conn, rec, { allowUpdate: true });
    await conn.commit();
    return recordId;
  } catch (err) {
    await conn.rollback();
...
async function upsertRecord(conn, rec, opts = {}) {
  ...
  if (!recordId) {
    const [r] = await conn.query(
      `INSERT INTO wb_research_records
        (title, abstract, authors_json, journal, year, volume, issue, pages, language, study_type,
      ...
      [
        ...
        rec.year || null, rec.volume || null, rec.issue || null, rec.pages || null,
        rec.language || null, rec.study_type || 'unknown',
    ...
    // Ghi đè bằng dữ liệu MỚI — COALESCE giữ giá trị cũ chỉ khi giá trị mới rỗng (tránh
    // 1 lần nhập thiếu trường vô tình xoá mất dữ liệu tốt đã có từ lần trước).
    await conn.query(
      `UPDATE wb_research_records SET
         title = ?, abstract = COALESCE(?, abstract), authors_json = ?,
         journal = COALESCE(?, journal), year = COALESCE(?, year),
         volume = COALESCE(?, volume), issue = COALESCE(?, issue), pages = COALESCE(?, pages),
         language = COALESCE(?, language),
         study_type = IF(? <> 'unknown', ?, study_type),
         subjects_json = ?, keywords_json = ?, merged_from_json = ?
       WHERE id = ?`,
      [
        rec.title.slice(0, 700), rec.abstract || null,
        JSON.stringify(rec.authors || []), rec.journal || null, rec.year || null,
        rec.volume || null, rec.issue || null, rec.pages || null, rec.language || null,
        rec.study_type || 'unknown', rec.study_type || 'unknown',
        JSON.stringify(rec.subject_headings || []), JSON.stringify(rec.keywords || []),
        JSON.stringify(rec.merged_from || []),
        recordId,
      ]
    );
  }
