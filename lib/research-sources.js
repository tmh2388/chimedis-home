...
async function searchSemanticScholar(query, { page = 1, perPage = 20, advanced } = {}) {
  ...
  // Timeout ngắn hơn: nguồn bổ sung, không có khoá dễ bị 429 — không nên làm chậm cả lượt tìm kiếm.
  const data = await getJson(`https://api.semanticscholar.org/graph/v1/paper/search?${params.toString()}`, headers, 6000);
