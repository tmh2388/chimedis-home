...
// Khi có truy vấn thực, các khoá này KHÔNG gửi thẳng lên nguồn. Lý do: OpenAlex/Europe PMC
// tìm theo kiểu "tài liệu chứa đủ từ khoá" trên cả toàn văn → tập ứng viên rất rộng và lỏng.
// Nếu bảo nguồn sắp thẳng theo trích dẫn, các bài SIÊU nhiều trích dẫn nhưng chỉ khớp lỏng
// (tổng quan gánh nặng bệnh tật toàn cầu, hướng dẫn báo cáo TIDieR/PRISMA/CONSORT...) sẽ nhảy
// lên đầu dù không đúng chủ đề. Thay vào đó: lấy theo ĐỘ LIÊN QUAN, rồi mergeAndRank() sắp lại
// theo trích dẫn TRONG tập đã lọc liên quan.
const RELEVANCE_GATED_SORTS = new Set(['citations', 'citations_asc']);
// Với các khoá trên: chỉ sắp theo trích dẫn TRONG nhóm N tài liệu liên quan nhất. Nhờ vậy
// "sắp theo trích dẫn" = "bài có ảnh hưởng nhất VỀ CHỦ ĐỀ NÀY", không phải "bài nhiều trích
// dẫn nhất có chứa loáng thoáng từ khoá" (hướng dẫn lâm sàng lớn, tổng quan gánh nặng bệnh...).
const RELEVANCE_WINDOW = 50;
...
async function searchOpenAlex(query, { page = 1, perPage = 20, yearFrom, yearTo, openAccessOnly, pubmedOnly, pmcOnly, docType, sort, advanced } = {}) {
  ...
  const hasSearch = !!(searchParam && searchParam.trim());
  let sortKey = sort && OA_SORT[sort] ? sort : 'relevance';
  // Có truy vấn + xin sắp theo trích dẫn → vẫn LẤY theo độ liên quan (lọc đúng chủ đề trước),
  // việc sắp theo trích dẫn để mergeAndRank() làm trên tập đã lọc.
  if (hasSearch && RELEVANCE_GATED_SORTS.has(sortKey)) sortKey = 'relevance';
  if (sortKey === 'relevance' && !hasSearch) {
...
async function searchEuropePmc(query, { page = 1, perPage = 20, yearFrom, yearTo, openAccessOnly, pubmedOnly, pmcOnly, docType, sort, advanced } = {}) {
  ...
  // Có truy vấn + xin sắp theo trích dẫn → giữ thứ tự ĐỘ LIÊN QUAN mặc định của Europe PMC,
  // để mergeAndRank() sắp lại theo trích dẫn trên tập đã lọc đúng chủ đề (xem RELEVANCE_GATED_SORTS).
  const gateThisSort = ((query && query.trim()) || (advanced && advanced.length)) && RELEVANCE_GATED_SORTS.has(sort);
  if (sort && EPMC_SORT[sort] && !gateThisSort) q += EPMC_SORT[sort];
  const params = new URLSearchParams({
    query: q,
    format: 'json',
...
async function searchCore(query, { page = 1, perPage = 20, yearFrom, yearTo, sort, advanced } = {}) {
  const raw = advanced && advanced.length ? advanced.map((r) => r.term).join(' ') : query;
  // CORE hỗ trợ cú pháp kiểu Elasticsearch trong `q`. Chữ Hán không có khoảng trắng giữa từ —
  // nếu để trần, bộ tách từ bên dưới CORE dễ bẻ vụn thành TỪNG KÝ TỰ đơn lẻ rồi khớp lỏng lẻo,
  // ra hàng triệu "kết quả" không liên quan (đã kiểm chứng thật: truy vấn "醒脑开窍 中风" khớp
  // nhầm một bài về nơ-ron não thỏ chỉ vì trùng rời rạc ký tự "脑"/"中"). Đặt mỗi cụm CÓ CHỮ HÁN
  // trong ngoặc kép để ép CORE khớp nguyên cụm từ thay vì từng ký tự rời.
  let q = raw.split(/\s+/).filter(Boolean).map((w) => (isCJK(w) ? `"${w}"` : w)).join(' ');
...
export async function searchAll(query, opts = {}) {
  ...
  settled.forEach((outcome, i) => {
    ...
    outcome.value.results.forEach((item, idx) => {
      const key = item.doi
        ? `doi:${item.doi}`
        : `title:${item.title.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim().slice(0, 120)}`;
      const existing = merged.get(key);
      if (!existing) {
        // `_rel` = hạng theo độ liên quan trong nguồn (nguồn đã trả đúng thứ tự). Bài xuất hiện
        // ở nhiều nguồn giữ hạng NHỎ nhất.
        merged.set(key, { ...item, sources: [item.source], _rel: idx });
        return;
      }
      existing._rel = Math.min(existing._rel ?? idx, idx);
  ...
  // "Sắp theo trích dẫn" trên truy vấn thực: thu hẹp về N bài liên quan nhất TRƯỚC, rồi mới
  // sắp theo trích dẫn. Tránh việc bài chục nghìn trích dẫn nhưng chỉ khớp lỏng nhảy lên đầu.
  const gated =
    ((query && query.trim()) || (opts.advanced && opts.advanced.length)) &&
    RELEVANCE_GATED_SORTS.has(opts.sort);
  if (gated) {
    pool.sort((a, b) => (a._rel ?? 1e9) - (b._rel ?? 1e9));
    pool = pool.slice(0, RELEVANCE_WINDOW);
  }
  const results = pool.sort(sorters[opts.sort] || sorters.relevance);

  return { results, perSource, errors };
}

// Xuất từng bộ nối đơn nguồn để lớp connector abstraction (lib/connectors/) tái dùng
// mà KHÔNG phải viết lại logic truy vấn/chuẩn hoá. Chữ ký: fn(query, opts) -> { total, results[] }
// với results[] theo shape chuẩn nội bộ (source,id,pmid,doi,title,authors,year,venue,...).
export { searchOpenAlex, searchEuropePmc, searchCore, searchSemanticScholar };
