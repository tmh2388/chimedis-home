...
export function parseNoteExpress(text) {
  ...
  for (const line of lines) {
    ...
    if (!hit) { lastSlot = null; continue; }
    if (ARRAY_FIELDS.has(hit.canon)) {
      let vals = value.split(/[;；]/).map((s) => s.trim()).filter(Boolean);
      // SinoMed {Author} gắn số chú thích đơn vị dính liền cuối tên (vd "孙永旗(1)",
      // "张玉生(3,4)" — khác CNKI/万方 vốn tách riêng ở {Author Affiliation}), file thật
      // user cung cấp 2026-09-19. Bỏ số này khỏi authors — KHÔNG áp cho keywords (canon
      // khác), tránh cắt nhầm nội dung trong ngoặc của từ khoá thật.
      if (hit.canon === 'authors') vals = vals.map((v) => v.replace(/\(\d+(?:[,，]\d+)*\)$/, '').trim());
      acc.arrays[hit.canon].push(...vals);
      lastSlot = { canon: hit.canon, isArray: true };
    } else { setField(acc, hit.canon, hit.priority, value); lastSlot = { canon: hit.canon, isArray: false }; }
  }
  if (acc && acc.scalar.title) records.push(finalizeAccumulator(acc));
