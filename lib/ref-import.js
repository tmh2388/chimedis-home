...
export function parseNoteExpress(text) {
  ...
  let lastSlot = null;
  for (const line of lines) {
    // KHÔNG neo `^` ở đầu dòng: file .net CNKI thật gặp trường hợp record ĐẦU TIÊN dính
    // liền vào cuối 1 đoạn <script> nội bộ của trang (vd `...}();        ;{Reference
    // Type}: Conference Proceedings`, không có xuống dòng) — neo cứng đầu dòng làm mất
    // trắng bản ghi đó (bug thật đã gặp, file export thật của user, 2026-09-12).
