...
const FIELD_ALIASES = {
  ...
  // của luận văn (publisher) sau cùng — chỉ dùng khi không có gì tốt hơn.
  journal: {
    ris: ['JO', 'JF', 'T2', 'J2'], endnote: ['B', 'J'],
  ...
  doi: { ris: ['DO'], endnote: [], noteexpress: ['doi'] },
...
function finalizeAccumulator(acc) {
  ...
  return {
    ...
    keywords: acc.arrays.keywords, url: g('url'),
  };
}
...
export function parseRis(text) {
  ...
  for (const rawLine of lines) {
    ...
    const [, tag, value] = m;
    if (tag === 'TY') { acc = newAccumulator(); lastSlot = null; continue; }
    if (!acc) continue;
...
export function parseEndnoteTagged(text) {
  ...
  for (const line of lines) {
    ...
    const [, tag, value] = m;
    if (tag === '0') { if (acc && acc.scalar.title) records.push(finalizeAccumulator(acc)); acc = newAccumulator(); lastSlot = null; continue; }
    if (!acc) continue;
...
export function parseNoteExpress(text) {
  ...
  for (const line of lines) {
    ...
    if (key === 'reference type') { if (acc && acc.scalar.title) records.push(finalizeAccumulator(acc)); acc = newAccumulator(); lastSlot = null; continue; }
    if (!acc) continue;
