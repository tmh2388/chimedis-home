...
// D6: trần cứng số lượt gọi LLM dịch / ngày (chặn chi phí — reviewer PR#4 yêu cầu quota).
const MAX_CALLS_PER_DAY = parseInt(process.env.LLM_TRANSLATE_DAILY_MAX || '400', 10);
...
let dayKey = '';
let dayCount = 0;
export function llmCallsToday() { return { day: dayKey, count: dayCount, cap: MAX_CALLS_PER_DAY }; }
...
export async function translateQueryTerms(rawQuery, knownTerms = []) {
  ...
  const p = (async () => {
    ...
    const user =
      ...
      'CHỈ dịch đúng những gì được viết — KHÔNG thêm khái niệm không có trong cụm gốc. ' +
      'Tối đa 3 phần tử, chọn các thuật ngữ ĐẶC THÙ nhất (bệnh/dược liệu/phương pháp cụ thể); ' +
      'bỏ qua từ nối (điều trị/nghiên cứu/hiệu quả/kết hợp/thử nghiệm...). Không bịa. Chỉ JSON.';
