...
// và có token viết-Hoa-đầu liền kề (trước/sau) ⇒ nhiều khả năng là tên người
...
function matchLatin(tokens, idx, pinnedTerms, rawTokens) {
  ...
  for (let size = Math.min(8, n); size >= 1; size--) {
    for (let i = 0; i + size <= n; i++) {
      ...
      // R2: khớp ĐƠN TOKEN tới concept clinical, mà token trông như tên riêng
      // (Hoa-đầu + hàng xóm Hoa-đầu) → bỏ qua, để rơi xuống unresolved (giữ nguyên văn).
