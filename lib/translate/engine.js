...
function matchLatin(tokens, idx, pinnedTerms) {
  ...
  // Tầng 1 — exact theo dấu, dài trước ngắn
  for (let size = Math.min(6, n); size >= 1; size--) {
  ...
  // Tầng 2 — folded n-gram cho span còn trống (dài trước ngắn), tối đa 3 từ
  for (let size = Math.min(3, n); size >= 1; size--) {
    for (let i = 0; i + size <= n; i++) {
      if (taken.slice(i, i + size).some(Boolean)) continue;
      const phrase = tokens.slice(i, i + size).join(' ');
      const folded = foldDiacritics(phrase);
...
const VI_STOP = new Set(['va', 'voi', 'cua', 'trong', 'sau', 'truoc', 'khi', 'cho', 'la', 'co', 'khong',
  'bang', 'de', 've', 'tren', 'duoi', 'hay', 'hoac', 'nhung', 'cac', 'mot', 'nghien', 'cuu', 'danh', 'gia',
  'tac', 'dung', 'hieu', 'qua', 'ket', 'hop', 'vai', 'tro', 'benh', 'nhan', 'nguoi', 'lon', 'tre']);
