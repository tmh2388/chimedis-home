...
function matchLatin(tokens, idx, pinnedTerms, rawTokens) {
  ...
  for (let size = Math.min(8, n); size >= 1; size--) {
    for (let i = 0; i + size <= n; i++) {
      ...
      // R2: khớp ĐƠN TOKEN tới concept clinical, mà token đó trông như tên riêng
      // (Hoa-đầu + hàng xóm Hoa-đầu) → bỏ qua, để token rơi xuống unresolved (giữ nguyên văn).
      if (size === 1 && looksLikeProperNoun(rawTokens, i) &&
          ids.some((id) => idx.byId.get(id)?.clinical)) {
        continue;
      }
...
// R1 (blind v1 S0): so khớp trên DẠNG ĐÚNG DẤU — KHÔNG fold — nếu không, từ có nghĩa
// mà bỏ dấu trùng hư từ (`khí`→khi, `nhiễu`→nhieu, `đo`→do, `bằng`(dụng cụ)→băng, `bơi`→bởi)
// bị xoá âm thầm khỏi truy vấn = tái sinh lỗi châm/chàm ở tầng stopword.
// CHỈ hư từ ngữ pháp thuần; KHÔNG gồm hình vị y khoa (cứu/giả/hợp/vai...) — giữ cụm chưa dịch nguyên văn.
// `có` cố ý KHÔNG có ở đây (mơ hồ copula/"có", để nguyên văn an toàn hơn).
const VI_STOP_EXACT = new Set([
  'và', 'với', 'của', 'trong', 'sau', 'trước', 'khi', 'cho', 'là', 'bằng',
  'để', 'về', 'trên', 'dưới', 'hay', 'hoặc', 'những', 'các', 'một', 'nhiều', 'do', 'bởi',
]);

// Token cấu trúc truy vấn (R3) — giữ NGUYÊN trong effective_query, KHÔNG tính là "chưa dịch".
const STRUCT_TOKEN = new Set(['(', ')', 'AND', 'OR', 'NOT']);

// R2 (blind v1 S1-1): heuristic tên riêng — token viết Hoa-đầu, chữ còn lại thường,
// và có token viết-Hoa-đầu liền kề (trước hoặc sau) ⇒ nhiều khả năng là tên người
// ("Trần Thị Châm") → KHÔNG dịch dù khớp concept clinical đơn-âm-tiết.
function isTitleCase(t) {
  return typeof t === 'string' && /^\p{Lu}[\p{Ll}̀-ͯ]*$/u.test(t);
}
function looksLikeProperNoun(rawTokens, i) {
  if (!rawTokens || !isTitleCase(rawTokens[i])) return false;
  return isTitleCase(rawTokens[i - 1]) || isTitleCase(rawTokens[i + 1]);
}
...
export function translate(rawQuery, { mode = 'discovery', pinnedTerms = {}, indexes } = {}) {
  ...
  // `rawNormalized` cùng độ dài với `normalized` (chỉ khác hoa/thường) ⇒ cắt cùng offset (R2).
  const segments = [];
  const runs = cjkRuns(norm.normalized);
  let cursor = 0;
  for (const run of runs) {
    if (run.start > cursor) segments.push({ type: 'latin', text: norm.normalized.slice(cursor, run.start).trim(), raw: norm.rawNormalized.slice(cursor, run.start).trim() });
    segments.push({ type: 'cjk', text: run.text });
    cursor = run.start + run.text.length;
  }
  if (cursor < norm.normalized.length) segments.push({ type: 'latin', text: norm.normalized.slice(cursor).trim(), raw: norm.rawNormalized.slice(cursor).trim() });
  if (!runs.length) segments.length = 0, segments.push({ type: 'latin', text: norm.normalized, raw: norm.rawNormalized });
  ...
  for (const seg of segments) {
    ...
    if (seg.type === 'cjk') {
      ...
      const rawToks = (seg.raw || '').split(/\s+/).filter(Boolean);
  ...
  for (const s of allSpans) {
    ...
    } else if (STRUCT_TOKEN.has(s.text)) {
      // R3: toán tử Boolean / ngoặc — GIỮ NGUYÊN (viết HOA sẵn), KHÔNG tính "chưa dịch".
      outParts.push(s.text);
    } else {
      // unresolved — bỏ hư từ tiếng Việt (so khớp DẠNG ĐÚNG DẤU, R1) nhưng KHÔNG coi là dịch;
      // vẫn giữ cụm "có nghĩa" nguyên văn (review#2)
      if (VI_STOP_EXACT.has(s.text)) continue;
      outParts.push(s.text);
