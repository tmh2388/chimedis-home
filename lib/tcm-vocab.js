...
export function buildSearchQuery(rawQuery, opts = {}) {
  ...
  // Cắt bớt: nếu quá MAX_QUERY_GROUPS nhóm → bỏ nhóm "generic" trước (giữ nguyên thứ tự nhóm
  // đặc thù: dược liệu / bệnh / huyệt / pháp trị cụ thể).
  if (groups.length > MAX_QUERY_GROUPS) {
    const specific = groups.filter((g) => !g.generic);
    const generic = groups.filter((g) => g.generic);
    groups = [...specific, ...generic].slice(0, MAX_QUERY_GROUPS);
  }
  ...
  if (remainder) {
    ...
    remainderIdx.forEach((idx) => {
      ...
      if (isCJK) {
        const unmatched = [...original].filter((ch) => /[㐀-鿿豈-﫿]/.test(ch) && !hanConsumed.includes(ch));
