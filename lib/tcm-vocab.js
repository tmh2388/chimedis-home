...
import * as translateEngineV2 from './translate/index.js'; // Milestone D — lazy: chỉ nạp chỉ mục khi TRANSLATE_ENGINE=v2|shadow
import { logShadowDiff } from './translate/shadow.js';
import { logShadowToDb } from './translate/shadow-log.js';
...
const GENERAL = {
  ...
  'trung phong':             { en: 'stroke', syn: ['cerebrovascular accident'] }, // 中风 — thắng khoá huyệt "Trung Phong" (LR4) của CoreDB
  'trung phong nao':         { en: 'stroke', syn: ['cerebrovascular accident'] },
  ...
  'benh tri':                { en: 'hemorrhoids', syn: [] }, // KHÔNG dùng khoá 'tri' — trùng "trị" (điều trị) sau khi bỏ dấu
  'tri ngoai':               { en: 'external hemorrhoids', syn: [] },
  ...
  'tri lieu':                { en: 'therapy', syn: ['treatment'] },
  'dieu tri':                { en: 'treatment', syn: ['therapy'] },
  'lieu phap':               { en: 'therapy', syn: [] },
  'du phong':                { en: 'prevention', syn: ['prophylaxis'] },
  'phong ngua':              { en: 'prevention', syn: ['prophylaxis'] },
...
const ZH_EXTRA = {
  ...
  // từ nối y khoa hay đi kèm trong truy vấn tiếng Trung — cần bản dịch + đồng nghĩa tốt
  '预防':   { en: 'prevention', syn: ['prophylaxis', 'preventive'] },
  '治疗':   { en: 'treatment', syn: ['therapy', 'therapeutic'] },
  '疗效':   { en: 'efficacy', syn: ['effectiveness', 'therapeutic effect'] },
  '临床':   { en: 'clinical', syn: [] },
  '诊断':   { en: 'diagnosis', syn: ['diagnostic'] },
  '机制':   { en: 'mechanism', syn: ['mechanisms', 'molecular mechanism'] },
  '随机对照': { en: 'randomized controlled trial', syn: ['RCT', 'clinical trial'] },
  '综述':   { en: 'review', syn: ['systematic review'] },
  '系统评价': { en: 'systematic review', syn: ['meta-analysis'] },
  '荟萃分析': { en: 'meta-analysis', syn: ['systematic review'] },
  '证候':   { en: 'syndrome', syn: ['pattern', 'TCM syndrome'] },
  '预后':   { en: 'prognosis', syn: ['outcome'] },
  '康复训练': { en: 'rehabilitation training', syn: ['rehabilitation'] },
  '联合':   { en: 'combined therapy', syn: ['combination therapy'] },
  '联用':   { en: 'combined therapy', syn: [] },
  '试验':   { en: 'trial', syn: ['clinical trial'] },
  '研究':   { en: 'research', syn: ['study'] },
  '观察':   { en: 'observation', syn: ['clinical observation'] },
  '疗程':   { en: 'treatment course', syn: [] },
  '有效性': { en: 'effectiveness', syn: ['efficacy'] },
  '安全性': { en: 'safety', syn: [] },
};

// Bảng mở rộng ĐỒNG NGHĨA TIẾNG ANH — áp SAU khi đã dịch xong, bất kể cụm đến từ lớp từ điển
// nào. Giải quyết vấn đề "1 thuật ngữ Trung/Việt chỉ ra 1 từ tiếng Anh trong khi PubMed dùng
// nhiều biến thể" (vd 预防 → chỉ "prophylaxis" trong khi phần lớn bài dùng "prevention").
// Khoá: dạng tiếng Anh bất kỳ (đã hạ chữ thường). Giá trị: các biến thể HAY GẶP trong PubMed.
const EN_SYNONYMS = {
  prevention: ['prophylaxis', 'preventive', 'prophylactic'],
  prophylaxis: ['prevention', 'preventive', 'prophylactic'],
  preventive: ['prevention', 'prophylaxis'],
  treatment: ['therapy', 'therapeutic', 'management'],
  therapy: ['treatment', 'therapeutic'],
  therapeutic: ['treatment', 'therapy'],
  efficacy: ['effectiveness', 'therapeutic effect', 'clinical efficacy'],
  effectiveness: ['efficacy', 'clinical effectiveness'],
  mechanism: ['mechanisms', 'molecular mechanism', 'signaling pathway'],
  rehabilitation: ['recovery', 'functional recovery'],
  recovery: ['rehabilitation', 'functional recovery'],
  tumor: ['tumour', 'neoplasm', 'cancer', 'carcinoma'],
  tumour: ['tumor', 'neoplasm', 'cancer'],
  neoplasm: ['tumor', 'cancer', 'carcinoma', 'malignancy'],
  ...
  immunity: ['immune function', 'immune response', 'immunomodulation'],
  acupuncture: ['electroacupuncture', 'manual acupuncture', 'needling'],
  moxibustion: ['moxa'],
  obesity: ['overweight', 'body weight'],
  osteoporosis: ['bone loss', 'bone mineral density'],
  arthritis: ['joint inflammation'],
  osteoarthritis: ['degenerative joint disease'],
  'rheumatoid arthritis': ['RA'],
  'quality of life': ['QoL', 'wellbeing'],
  elderly: ['aged', 'older adults', 'geriatric'],
  children: ['pediatric', 'paediatric', 'childhood'],
  pregnancy: ['pregnant', 'gestation'],
  diagnosis: ['diagnostic', 'differential diagnosis'],
  prognosis: ['outcome', 'prognostic'],
  incidence: ['prevalence', 'epidemiology'],
  complication: ['complications', 'adverse effect', 'sequelae'],
  fatigue: ['tiredness', 'chronic fatigue'],
  cognition: ['cognitive function', 'cognitive impairment'],
  apoptosis: ['programmed cell death'],
  'oxidative stress': ['reactive oxygen species'],
  'gut microbiota': ['intestinal microbiome', 'gut microbiome', 'intestinal flora'],
  'herbal medicine': ['herbal formula', 'phytotherapy', 'botanical medicine'],
  syndrome: ['pattern', 'symptom complex'],
};

/**
 * Ghép `en` + đồng nghĩa sẵn có + biến thể PubMed (EN_SYNONYMS), khử trùng lặp không phân biệt
 * hoa/thường, giới hạn tổng số nhánh OR (tránh truy vấn quá dài).
 */
export function expandEnSynonyms(en, existingSyn = [], max = 5) {
  const out = [];
  const seen = new Set();
  const add = (s) => {
  ...
  add(en);
  for (const s of existingSyn) add(s);
  const extra = EN_SYNONYMS[String(en).trim().toLowerCase()] || [];
...
// ===== Lớp phủ TỰ HỌC (GĐ2) =====
// Các cụm do LLM dịch từ lưu lượng thật, đã "auto_active"/"approved" trong bảng dict_candidates.
// Nạp lúc khởi động + làm mới định kỳ (xem lib/dict-learn.js). Ưu tiên THẤP hơn INDEX (từ điển
// đã rà soát tay + CoreDB luôn thắng). App vẫn chạy bình thường nếu lớp phủ rỗng / không có DB.
const OVERLAY = new Map();
let OVERLAY_HAN = new Set();
let OVERLAY_HAN_MAXLEN = 2;

/** entries: [{ key: <đã chuẩn hoá HOẶC chuỗi Hán thô>, en, syn:[] }] */
export function setLearnedOverlay(entries) {
  OVERLAY.clear();
  for (const e of entries || []) {
    if (!e || !e.key || !e.en) continue;
    OVERLAY.set(e.key, { en: e.en, syn: Array.isArray(e.syn) ? e.syn : [] });
  }
  OVERLAY_HAN = new Set([...OVERLAY.keys()].filter((k) => /[㐀-鿿豈-﫿]/.test(k)));
  OVERLAY_HAN_MAXLEN = Math.max(2, ...[...OVERLAY_HAN].map((k) => k.length));
}
function lookup(key) {
  return INDEX.get(key) || OVERLAY.get(key) || null;
}
...
export function matchTcmTerms(query) {
  ...
  if (!raw.trim()) return { hits: [], remainder: '', remainderIdx: [], hanConsumed: '' };
  const norm = normalizeTerm(raw);
  const words = norm ? norm.split(' ') : [];
  const hits = [];
  const usedRanges = [];
  // Truy vấn TOÀN chữ Latin (không dấu, không chữ Hán) — nhiều khả năng người dùng gõ tiếng Anh.
  // Khi đó KHÔNG khớp khoá 1 từ ngắn ≤4 ký tự (tránh "gut"→gout, "cell"/"cold"/"wind"→thuật
  // ngữ YHCT). Cụm nhiều từ hoặc từ dài vẫn khớp (vd "danshen", "buzhong yiqi tang").
  const allAscii = /^[\x00-\x7f]+$/.test(raw);
  for (let size = Math.min(6, words.length); size >= 1; size--) {
    for (let i = 0; i + size <= words.length; i++) {
      if (usedRanges.some(([s, e]) => i < e && i + size > s)) continue;
      const phrase = words.slice(i, i + size).join(' ');
      if (allAscii && size === 1 && phrase.length <= 4) continue;
      const entry = lookup(phrase);
      if (entry) {
  ...
  let hanConsumed = ''; // các ký tự Hán ĐÃ khớp — để tầng trên khỏi cảnh báo nhầm "chưa dịch"
  for (const run of raw.match(/[㐀-鿿豈-﫿]+/g) || []) {
    ...
    while (i < run.length) {
      ...
      const maxLen = Math.max(HAN_KEY_MAXLEN, OVERLAY_HAN_MAXLEN);
      for (let len = Math.min(maxLen, run.length - i); len >= 2; len--) {
        const sub = run.slice(i, i + len);
        if (HAN_KEYS.has(sub) || OVERLAY_HAN.has(sub)) { matched = sub; break; }
      }
      if (matched) {
        hanConsumed += matched;
        if (!seen.has(matched)) {
          const entry = lookup(matched);
  ...
  return { hits, remainder: remainderWords.join(' ').trim(), remainderIdx, hanConsumed };
}
 ...
 */
// Thuật ngữ "chung chung" — hữu ích để hiểu ý nhưng KHÔNG nên bắt buộc AND (bài nào cũng có
// "treatment"/"efficacy"...). Khi truy vấn dài, đây là nhóm bị bỏ trước để tránh AND quá chặt.
const GENERIC_EN = new Set([
  'treatment', 'therapy', 'therapeutic', 'management', 'efficacy', 'effectiveness',
  'clinical', 'clinical study', 'clinical trial', 'randomized controlled trial',
  'trial', 'study', 'research', 'review', 'systematic review', 'meta-analysis',
  'mechanism', 'combined therapy', 'combination therapy', 'combined', 'prevention',
  'diagnosis', 'prognosis', 'observation', 'analysis', 'syndrome', 'disease',
  'treatment course', 'safety', 'evaluation', 'assessment', 'clinical observation',
  'combined treatment', 'course of treatment',
]);
// Số nhóm OR tối đa trong 1 truy vấn hiệu dụng — quá nhiều nhóm AND → gần như không ra kết quả.
const MAX_QUERY_GROUPS = 6;

// Hư từ / liên từ / cụm "chung chung" tiếng Việt (đã bỏ dấu) — phần chữ còn sót khớp bộ này
// thì BỎ HẲN: không đẩy vào truy vấn API, cũng không báo "chưa dịch" (tránh gọi LLM thừa +
// tránh chèn "trong", "sau"... vào chuỗi tìm kiếm tiếng Anh).
const VI_STOP = new Set([
  'trong', 'ngoai', 'sau', 'truoc', 'cua', 'va', 'voi', 'cac', 'nhung', 'cho', 'khi', 've',
  'tai', 'do', 'bang', 'duoc', 'mot', 'la', 'co', 'khong', 'nhu', 'den', 'tu', 'boi', 'theo',
  'hay', 'hoac', 'nham', 'giua', 'moi', 'nao', 'gi', 'ma', 'thi', 'se', 'da', 'dang', 'bi',
  'tac', 'dung', 'tac dung', 'hieu', 'qua', 'hieu qua', 'ket', 'hop', 'ket hop', 'phoi hop',
  'dieu', 'tri', 'dieu tri', 'nghien', 'cuu', 'nghien cuu', 'danh', 'gia', 'danh gia',
  'khao', 'sat', 'khao sat', 'anh', 'huong', 'anh huong', 'vai', 'tro', 'vai tro',
  'phuong', 'phap', 'phuong phap', 'lam sang', 'thu', 'nghiem', 'thu nghiem', 'tong', 'quan',
  'tong quan', 'phan', 'tich', 'phan tich', 'quan sat', 'lieu', 'trinh', 'lieu trinh',
  'an', 'toan', 'an toan', 'kha', 'nang', 'moi lien', 'lien', 'muc', 'cai thien',
]);

export function buildSearchQuery(rawQuery, opts = {}) {
  const { orSynonyms = false, mode = 'discovery', pinnedTerms = {} } = opts;

  // Milestone D — engine dịch v2 (giữ dấu tiếng Việt). Cờ env TRANSLATE_ENGINE:
  //   legacy (mặc định) — hành vi cũ, không đụng gì
  //   shadow            — tính v2 + đính kèm `_shadow` để đối chiếu, VẪN trả kết quả legacy
  //   v2                — dùng kết quả v2 (sau khi qua GATE §12)
  // Import động để không kéo engine vào mọi caller khi chưa bật.
  const teMode = String(process.env.TRANSLATE_ENGINE || 'legacy').toLowerCase();
  if (teMode === 'v2' || teMode === 'shadow') {
    try {
      const r = translateEngineV2.translateQuery(rawQuery, { mode, pinnedTerms });
      const v2result = {
        text: r.effective_query,
        expandedFrom: r.translated.map((t) => ({ vi: t.from, en: t.to, syn: [], confidence: t.confidence })),
        note: r.warning || 'Đã dịch thuật ngữ y khoa (engine v2, giữ dấu).',
        untranslated: r.unresolved,
        // trường mới (§8) — caller M1/H2 đọc thêm:
        spans: r.spans,
        disambiguation: r.disambiguation,
        needs_confirmation: r.needs_confirmation || [],
        needs_resolution: r.needs_resolution,
        engine_version: r.engine_version,
        confidenceCounts: r.translated.reduce((a, t) => ((a[t.confidence] = (a[t.confidence] || 0) + 1), a), {}),
      };
      if (teMode === 'v2') return v2result;
      // shadow: rơi xuống nhánh legacy, nhưng đính kèm để log/đối chiếu (review PR#4)
      opts.__shadow = v2result;
    } catch (err) {
      // engine lỗi → luôn fallback legacy, không làm hỏng tìm kiếm.
      // P0 rollout: LUÔN ghi 1 dòng structured để không "mù" khi shadow không đẻ diff.
      try {
        console.log(JSON.stringify({ evt: 'translate_v2_error', mode: teMode, msg: String(err && err.message || err).slice(0, 300) }));
      } catch { /* nuốt */ }
    }
  }

  // shadow: khi có kết quả v2 đính kèm, ghi structured diff rồi trả kết quả LEGACY nguyên vẹn.
  const _finish = (legacyResult) => {
    if (opts.__shadow) {
      logShadowDiff(legacyResult, opts.__shadow, { searchRunId: opts.searchRunId ?? null });
      // sink MySQL bền — best-effort/async, KHÔNG await, KHÔNG được làm fail search.
      logShadowToDb(legacyResult, opts.__shadow, {
        mode, testTag: opts.shadowTestTag ?? null, searchRunId: opts.searchRunId ?? null,
      });
    }
    return legacyResult;
  };

  const { hits, remainder, remainderIdx, hanConsumed = '' } = matchTcmTerms(rawQuery);
  const rawWords = String(rawQuery || '').split(/\s+/).filter(Boolean);

  if (!hits.length) {
    // Không khớp từ vựng nào — từ nào MANG DẤU TIẾNG VIỆT hoặc là CHỮ HÁN mà chưa dịch được
    // → đưa vào `untranslated` để tầng trên (LLM) xử lý, thay vì âm thầm gửi cho API chỉ hiểu
    // tiếng Anh (chữ Hán sau khi bỏ dấu KHÔNG đổi nên tuyệt đối không được coi là "tiếng Anh").
    const untranslated = rawWords.filter(
      (w) => w.length > 1 && (w !== stripDiacritics(w.normalize('NFD')) || /[㐀-鿿豈-﫿]/.test(w))
    ).filter((w) => !VI_STOP.has(stripDiacritics(w.normalize('NFD')).toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim()));
    return _finish({ text: String(rawQuery || '').trim(), expandedFrom: [], note: null, untranslated });
  }

  // Dựng nhóm OR cho từng thuật ngữ, kèm cờ "generic" để cắt bớt khi truy vấn quá dài.
  let groups = hits.map((h) => {
    const latinSyn = (h.syn || []).filter((s) => /^[\x00-\x7f]+$/.test(s) && !/[āáǎàēéěèīíǐìōóǒòūúǔù]/.test(s));
    let text;
    if (orSynonyms) {
      const alts = expandEnSynonyms(h.en, latinSyn);
      text = alts.length > 1 ? `(${alts.map((t) => `"${t}"`).join(' OR ')})` : `"${h.en}"`;
    } else {
      text = `"${h.en}"`;
    }
    return { text, generic: GENERIC_EN.has(String(h.en).trim().toLowerCase()) };
  });

  // Cắt nhóm để truy vấn không quá chặt:
  //  - Đã có ≥3 nhóm ĐẶC THÙ (dược liệu/bệnh/huyệt/pháp trị cụ thể) → BỎ HẲN nhóm "chung chung"
  //    (treatment/efficacy/trial/systematic review...). Người dùng lọc "Loại tài liệu" nếu cần.
  //  - Chưa đủ 3 nhóm đặc thù → giữ thêm nhóm chung để đủ ngữ cảnh.
  //  - Trần cứng MAX_QUERY_GROUPS trong mọi trường hợp.
  const specific = groups.filter((g) => !g.generic);
  const generic = groups.filter((g) => g.generic);
  groups = (specific.length >= 3 ? specific : [...specific, ...generic]).slice(0, MAX_QUERY_GROUPS);
  const parts = groups.map((g) => g.text);

  // Phần chữ còn sót: giữ lại nếu là tiếng Anh (gõ xen kẽ); phần chữ Hán / tiếng Việt có dấu
  // CHƯA khớp → báo "untranslated" (để LLM xử lý). Chữ Hán đã khớp trong `hanConsumed` thì bỏ qua.
  const untranslated = [];
  if (remainder) {
    const keep = [];
    remainderIdx.forEach((idx) => {
      ...
      if (!original) return;
      const isCJK = /[㐀-鿿豈-﫿]/.test(original);
      if (isCJK) {
        // Bỏ hư từ / liên từ tiếng Trung (的了和与及在对于并後后前及以为) — còn lại <2 ký tự
        // "có nghĩa" thì KHÔNG báo "chưa dịch" (tránh blob rác như "联合后的试验").
        const unmatched = [...original]
          .filter((ch) => /[㐀-鿿豈-﫿]/.test(ch) && !hanConsumed.includes(ch) && !'的了和與与及在對对於于並并後后前以為为者之'.includes(ch));
        if (unmatched.length >= 2) untranslated.push(unmatched.join(''));
        return;
      }
      const bare = stripDiacritics(original.normalize('NFD')).toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim();
      if (VI_STOP.has(bare)) return; // hư từ / cụm chung chung → bỏ hẳn
